...
foreach(glob(udaan_data_path('rooms/*.json'))?:[] as $f){
...
foreach(glob(udaan_data_path('imports/pending/*.json'))?:[] as $f){
    $raw=@file_get_contents($f);
    $d=is_string($raw)?json_decode($raw,true):null;
    if(!is_array($d)||(int)($d['expires_at']??0)<time()){if(@unlink($f))$pending++;}
}
foreach(glob(udaan_data_path('rate-limit/*.json'))?:[] as $f){
    $mtime=@filemtime($f);
    if(is_int($mtime)&&$mtime<(time()-86400)){if(@unlink($f))$rateLimits++;}
}
$userCache=$store->cleanupUserCacheFiles();
echo "Removed {$rooms} expired temporary room(s); {$pending} expired verified import snapshot(s); {$userCache} expired user-cache file(s); {$rateLimits} stale rate-limit file(s); {$unreadableRooms} unreadable room file(s) retained for investigation\n";
