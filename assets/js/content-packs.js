(()=>{
 ...
 async function sync(opts={}){
  ...
  const trust=await UdaanVault.get('content-trust');if(trust?.public_key&&trust.public_key!==m.public_key)throw new Error('Udaan content signing key changed; manual trust refresh required');
  await UdaanVault.set('content-trust',{version:1,public_key:m.public_key,key_id:m.signing_key_id,first_trusted_at:trust?.first_trusted_at||new Date().toISOString(),last_verified_at:new Date().toISOString(),source:'https-origin'});
  ...
  for(const meta of selected){
   ...
   if(!cached){const pr=await fetch(meta.url,{credentials:'same-origin',headers:{Accept:'application/json'}}),pj=await pr.json();if(!pr.ok||!pj.ok||!pj.pack)throw new Error(pj.error||`Pack unavailable: ${meta.pack_id}`);await verifyPack(pj.pack,pub,meta);cached={version:1,pack:pj.pack,verified_at:new Date().toISOString()};await UdaanVault.set(key,cached)}
   kept.push({pack_id:meta.pack_id,sha256:meta.sha256,key,cards:meta.cards,pillar:meta.pillar});total+=meta.cards||0;
  }
  await UdaanVault.set('content-pack-index',{version:1,manifest_sha256:m.manifest_sha256,bank_version:m.bank_version,key_id:m.signing_key_id,capacity:maxCards,card_count:total,packs:kept,updated_at:new Date().toISOString()});
  return {ok:true,cards:total,packs:kept.length,bank_version:m.bank_version,key_id:m.signing_key_id};
 }
 async function stats(){if(!window.UdaanVault)return null;return UdaanVault.get('content-pack-index')}
 window.UdaanContentPacks={sync,stats,verifyManifest,selectPacks};
})();
