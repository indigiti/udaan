(()=>{
 ...
 const schedule=()=>{if(!stopped){clearTimeout(timer);timer=setTimeout(draw,1500)}};
 async function draw(){
  if(stopped)return;if(document.hidden){schedule();return;}
  try{
   const r=await fetch(stateUrl,{cache:'no-store',credentials:'same-origin'});
   if(r.status===401||r.status===403){stopped=true;return;}
   const j=await r.json();if(!r.ok||!j.ok)return;const s=j.state;
 ...
 draw();
 document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!stopped){clearTimeout(timer);draw();}});
