...
function content_admin_key_path(): string { return udaan_data_path('content-admin.key'); }
function content_admin_key(): string {
    ...
    if (@file_put_contents($file, $key . "\n", LOCK_EX) === false) {
        throw new RuntimeException('Unable to initialize Content Admin access key. Make the runtime data directory writable.');
    }
    @chmod($file, 0600);
    return $key;
}
...
function save_verification(array $result): string {
    $token=uuid_v4();$dir=udaan_data_path('imports/pending');if(!is_dir($dir)&&!@mkdir($dir,0775,true)&&!is_dir($dir))throw new RuntimeException('Unable to create pending import folder.');
    $payload=['token'=>$token,'session_hash'=>hash('sha256',session_id()),'created_at'=>time(),'expires_at'=>time()+1800,'result'=>$result];
    $file=$dir.'/'.$token.'.json';if(@file_put_contents($file,json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT),LOCK_EX)===false)throw new RuntimeException('Unable to save verified import. Check runtime import permissions.');
    $s=&udaan_session();$s['content_import_pending']=$token;return $token;
}
function load_verification(string $token): array {
    if(!is_standard_uuid($token))throw new RuntimeException('Invalid verification token.');$file=udaan_data_path('imports/pending/'.$token.'.json');$raw=@file_get_contents($file);$p=is_string($raw)?json_decode($raw,true):null;
...
function cleanup_pending_verification(string $token): void { @unlink(udaan_data_path('imports/pending/'.$token.'.json'));$s=&udaan_session();if(($s['content_import_pending']??null)===$token)unset($s['content_import_pending']); }
function content_import_log_path(): string {return udaan_data_path('content/import-log.json');}
...
function prune_content_backups(int $keep=20): void {$dir=udaan_data_path('content/backups');$files=glob($dir.'/cards-*.json')?:[];usort($files,fn($a,$b)=>filemtime($b)<=>filemtime($a));foreach(array_slice($files,$keep) as $f)@unlink($f);}
