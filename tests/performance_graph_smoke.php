...
$add=function(string $id,string $date,string $arena,string $status,int $minutes,?int $actualMinutes=null)use(&$state):void{
    $state['missions'][$id]=[
        'id'=>$id,
        'scheduled_date'=>$date,
        'arena'=>$arena,
        ...
        'result'=>$actualMinutes===null?null:['actual_minutes'=>$actualMinutes],
    ];
};
...
$add('m7','2026-09-22','fit','completed',10,15);
...
if(($window['minutes']??0)!==100){fwrite(STDERR,"Completed minutes mismatch\n");exit(1);}
...
if(($window['arena_stats']['fit']['minutes']??0)!==25){fwrite(STDERR,"Fit arena minute aggregate mismatch\n");exit(1);}
...
if(($bests['best_minutes_day']['value']??0)!==40||($bests['best_minutes_day']['date']??'')!=='2026-09-22'){fwrite(STDERR,"Best minutes day mismatch\n");exit(1);}
