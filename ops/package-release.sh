...
for required in   "$STAGE/index.php"   "$STAGE/config.php"   "$STAGE/.htaccess"   "$STAGE/data/.htaccess"   "$STAGE/ops/.htaccess"   "$STAGE/data/content/cards.json"   "$STAGE/ready.php"   "$STAGE/health.php"   "$STAGE/ops/preflight.php"   "$STAGE/ops/rotate-runtime-security.php"   "$STAGE/lib/Player.php"   "$STAGE/lib/Mission.php"   "$STAGE/lib/DailyReadiness.php"   "$STAGE/lib/Fit.php"   "$STAGE/lib/Momentum.php"   "$STAGE/lib/Coach.php"   "$STAGE/lib/Social.php"   "$STAGE/lib/Competition.php"   "$STAGE/data/readiness/.gitkeep"   "$STAGE/data/social/.gitkeep"   "$STAGE/data/competition/.gitkeep"   "$STAGE/player.php"   "$STAGE/today.php"   "$STAGE/readiness.php"   "$STAGE/fit.php"   "$STAGE/momentum.php"   "$STAGE/coach.php"   "$STAGE/coach_action.php"   "$STAGE/connect.php"   "$STAGE/connect_action.php"   "$STAGE/team.php"   "$STAGE/team_action.php"   "$STAGE/leagues.php"   "$STAGE/league.php"   "$STAGE/league_action.php"   "$STAGE/mission_action.php"; do
  if [[ ! -f "$required" ]]; then
