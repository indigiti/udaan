...
fail_if_files "runtime learner/session state" bash -c "find '$STAGE/data/rooms' '$STAGE/data/history' '$STAGE/data/user-cache' '$STAGE/data/sync-outbox' '$STAGE/data/aggregates' '$STAGE/data/rate-limit' '$STAGE/data/insights' '$STAGE/data/players' '$STAGE/data/events' -type f ! -name '.gitkeep' -print 2>/dev/null"
...
for required in   "$STAGE/index.php"   "$STAGE/config.php"   "$STAGE/.htaccess"   "$STAGE/data/.htaccess"   "$STAGE/ops/.htaccess"   "$STAGE/data/content/cards.json"   "$STAGE/ready.php"   "$STAGE/health.php"   "$STAGE/ops/preflight.php"   "$STAGE/ops/rotate-runtime-security.php"   "$STAGE/lib/Player.php"   "$STAGE/player.php"   "$STAGE/today.php"; do
  if [[ ! -f "$required" ]]; then
