...
$events=is_array($payload['events']??null)?$payload['events']:[];if(count($events)>250)json_response(['ok'=>false,'error'=>'Too many offline events in one sync request'],422);$reserveId=is_string($payload['reserve_id']??null)?trim((string)$payload['reserve_id']):'';$result=$store->syncOfflineEvents((string)$auth['u'],(string)$auth['d'],$events,$reserveId);json_response(['ok'=>true]+$result);
